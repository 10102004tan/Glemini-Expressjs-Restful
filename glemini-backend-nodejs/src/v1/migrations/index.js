...
const quizSchema = new Schema({
  ...
  user_id: {
    ...
    default: new mongoose.Types.ObjectId('67175468870de15b55ce24b2'),
  },
  quiz_name: String,
  ...
  subject_ids: [
    {
      type: mongoose.Types.ObjectId,
      default: [new mongoose.Types.ObjectId('670f76d3fbe58e51feb81853')],
    },
  ],
  createdAt: { type: Date, default: Date.now },
  updatedAt: { type: Date, default: Date.now },
...
async function createRandomQuiz() {
  ...
  const questions = await Promise.all(
    Array.from({ length: 5 }, () => createRandomQuestion(quiz._id).then((q) => q.save())),
  );
  return quiz;
}
