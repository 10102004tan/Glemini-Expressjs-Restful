...
const updateStatusTeacher = async (user_id, teacher_status) => {
  try {
    const updatedStatus = await teacherModel.updateOne(
      { _id: user_id },
...
const updateImagesVerification = async (user_id, file_urls) => {
  const updatedImages = await teacherModel.updateOne(
    { _id: user_id },
    { $set: { file_urls: file_urls } },
  );
  return updatedImages;
};

const findStatusTeacher = async (user_id) => {
  const foundStatus = await teacherModel
    .findOne({ _id: user_id }, { teacher_status: 1, _id: 0 })
    .lean();
  return foundStatus;
};

const updateStatusTeacherV2 = async (user_id, teacher_status) => {
  try {
    const updatedStatus = await teacherModel.updateOne(
      { userId: user_id },
      { $set: { status: teacher_status } },
      { runValidators: true },
    );
    return updatedStatus;
  } catch (err) {
