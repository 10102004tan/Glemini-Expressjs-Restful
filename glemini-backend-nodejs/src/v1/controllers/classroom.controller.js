...
class ClassroomController {
	...
	async getClassroomsByTeacherId(req, res, next) {
		const classrooms = await classroomService.getClassroomsByTeacherId(
			req.body
		);
		return new OK({
			message: 'Fetch data classrooms successfully!',
	...
	async getClassroomsByStudentId(req, res, next) {
		const classrooms = await classroomService.getClassroomsByStudentId(
			req.body
		);
		return new OK({
	...
	async removeStudent(req, res, next) {
		...
		const result = await classroomService.removeStudentFromClass(
			classroomId,
			studentId
		);
		return new OK({
	...
	async notifyStudentWhenTeacherCreateRoom(req, res, next) {
		return new OK({
			...
			metadata: await classroomService.notifyStudentWhenTeacherCreateRoom(
				req.body
			),
		}).send(res);
	}
